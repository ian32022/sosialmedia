...
const createReport = async (req, res) => {
  try {
    ...
    let postId = null;
    if (finalPostUuid) {
      const [posts] = await pool.query('SELECT id FROM posts WHERE uuid = ?', [finalPostUuid]);
      if (posts.length === 0) return error(res, 'Post tidak ditemukan.', 404);
      postId = posts[0].id;
    }
...
// ─── GET SEMUA LAPORAN (moderator/admin) ─────────────────────
const getReports = async (req, res) => {
  try {
    ...
    return paginate(res, rows, { page, limit, total, total_pages: Math.ceil(total / limit) }, 'Daftar laporan berhasil diambil.');
  } catch (err) {
    return error(res, 'Terjadi kesalahan server.', 500);
  }
};

// ─── TINDAK LAPORAN (moderator/admin) ────────────────────────
const reviewReport = async (req, res) => {
  try {
    const { id } = req.params;
    const { action, status } = req.body;
    // action: hide_post | remove_post | ban_user | dismiss
    // status: reviewed | resolved | dismissed

    const [reports] = await pool.query(
      'SELECT r.*, p.user_id AS post_owner FROM reports r LEFT JOIN posts p ON p.id = r.post_id WHERE r.id = ?',
      [id]
    );
    if (reports.length === 0) return error(res, 'Laporan tidak ditemukan.', 404);

    const report = reports[0];

    if (action === 'hide_post' && report.post_id) {
      await pool.query("UPDATE posts SET status = 'hidden' WHERE id = ?", [report.post_id]);
    } else if (action === 'remove_post' && report.post_id) {
      await pool.query("UPDATE posts SET status = 'removed' WHERE id = ?", [report.post_id]);
    } else if (action === 'ban_user' && report.post_owner) {
      await pool.query('UPDATE users SET is_banned = 1 WHERE id = ?', [report.post_owner]);
    }
